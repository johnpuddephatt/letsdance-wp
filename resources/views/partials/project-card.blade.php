<a class="col-span-1 no-underline duration-500 group project-item" href="{{ get_permalink($project->ID) }}" data-aos
    ...
    <div class="transition-transform transform group-hover:scale-101">
        {!! get_the_post_thumbnail($project->ID, '2by1', [
        'class' => "block rounded border border-gray-50",
        'sizes' => '
        (min-width: 1536px) calc((1536px - 4rem) / 2),
        (min-width: 1280px) calc((1280px - 4rem) / 2),
        (min-width: 1024px) calc((1024ppx - 4rem) / 2),
        90vw'
        ]) !!}
    </div>
